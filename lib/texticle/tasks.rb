...
namespace :texticle do
  ...
  [:unaccent, :tsearch2].each do |mod|
    desc "Install #{mod} text search module"
    task :"install_#{mod}" => [:environment] do
      share_dir = `pg_config --sharedir`.chomp
      raise RuntimeError, 'cannot find Postgres\' shared directory' unless $?.exitstatus.zero?
      unaccent = "#{share_dir}/contrib/#{mod}.sql"
      unless system("ls #{unaccent}")
        raise RuntimeError, "cannot find #{mod} module; was it compiled and installed?"
      end
      begin
        ActiveRecord::Base.connection.execute(File.read(File.join(unaccent)))
        puts "#{mod} text search module successfully installed into the #{Rails.env} database."
      rescue Exception => e
        puts "#{e.class}: #{mod} has not been installed (already installed?). Skipping..."
      end
    end
  end
